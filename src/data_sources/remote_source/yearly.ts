...
export async function getYearly(id: number): Promise<Yearly> {
    const pg = `
    SELECT *
    ...
    `
    const response = await query(pg, oracle, [id]);
...
export async function getYearlyFiltered(id: number): Promise<Yearly> {
    const pg = `
    SELECT *
        FROM "elujoulisuseindeks"."aastased"
        WHERE "jykood" = $1
